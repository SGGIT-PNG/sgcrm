...
- **Apps Script에 `SG_CRM_Calendar_ADD_list.gs` 덧붙이고 새 버전 배포** (사용자 작업)
  → 그 후 웹훅 URL을 앱에 1회 입력 (이제 Firebase 저장이라 기기당 1회가 아닌 전체 1회)
  ※ `SG_CRM_Calendar_v8_Code.gs`는 신규 설치용 전체 코드. 기존 배포본 교체용으로는 쓰지 말 것
