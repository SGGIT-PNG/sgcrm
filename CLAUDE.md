...
4. **인라인 핸들러 ↔ window export 대조** ← 이 코드베이스 특유의 사고 유형
   ...
   **정적 HTML(275개)과 `render*`가 템플릿 리터럴로 만드는 마크업(72개) 양쪽**을 봅니다.
   (2026-09-03 보강 — 그 전에는 정적 HTML만 봐서 `toggleCat`·`cycleTodo`·`cycleWork` 등
   **JS에서만 쓰이는 함수 31개가 통째로 검사 사각지대**에 있었습니다)

실패 시 `exit code 1`. **검증 실패면 커밋하지 말고** 원인을 보고하고 롤백 여부를 확인합니다.

> ⚠️ 검사가 잡지 못하는 것: 문자열 연결로 조립되는 핸들러
> (`'onclick="'+fnName+'()"'` 처럼 함수명 자체가 변수인 경우)는 정적 분석이 불가능합니다.
> 그런 코드를 새로 만들면 export 여부를 눈으로 확인하세요.
