 ...
 *
 * 목적: CRM이 만든 일정 중 같은 이름이 2건 이상 겹쳐 있는 것을 찾아낸다.
 *
 * ⚠️ 2026-08-02 실행 결과: **중복 0건.** 애초에 없던 문제였다 (PROGRESS.md §4-② 참조).
 *    이 파일은 나중에 비슷한 의심이 생겼을 때 다시 쓰라고 남겨둔 진단 도구다.
 *
 * ⚠️ 프로젝트 이름 주의: 살아있는 것은 **`SG솔루션 CRM`** 이다.
 *    (`SG CRM 동기화`는 이름과 달리 방치된 옛 프로젝트)
 *
 * ⚠️ 이 파일은 **아무것도 삭제하거나 수정하지 않는다.** 읽어서 로그로 출력만 한다.
 *
 * ── 사용법 (배포 불필요) ──
 *  1. Apps Script 프로젝트 `SG솔루션 CRM` 을 연다
